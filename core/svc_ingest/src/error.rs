...
pub enum ApiError {
    ...
    #[error("bad request: {0}")]
    BadRequest(String),
    /// Route/feature exists but a later milestone owns the implementation --
    /// maps to 501. Used by the intake/connector seams this skeleton
    /// deliberately leaves unimplemented (see `src/lib.rs`).
...
mod tests {
    ...
    async fn internal_error_hides_detail() {
        let resp =
            ApiError::Internal(anyhow::anyhow!("secret connection string leaked")).into_response();
    ...
    async fn unimplemented_maps_to_501() {
        let resp = ApiError::Unimplemented("generic intake".into()).into_response();
        assert_eq!(resp.status(), StatusCode::NOT_IMPLEMENTED);
    }

    #[tokio::test]
    async fn not_found_maps_to_404() {
        let resp = ApiError::NotFound("source".into()).into_response();
        assert_eq!(resp.status(), StatusCode::NOT_FOUND);
    }

    #[tokio::test]
    async fn bad_request_maps_to_400() {
        let resp = ApiError::BadRequest("bad payload".into()).into_response();
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
    }

    #[tokio::test]
    async fn forbidden_maps_to_403() {
        let resp = ApiError::Forbidden("tenant mismatch".into()).into_response();
