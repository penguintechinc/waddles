...
| **DOKS** (gamma, production) | **Not available.** No native offering, and the installer DaemonSet does **not** work: DigitalOcean manages worker-node container-daemon configuration and its reconciler reverts node-level changes ("your changes are overwritten by the reconciler and do not persist" — official DOKS docs). No custom/BYO node image path exists in the docs, `doctl`, Terraform or Pulumi. | **Verified 2026-09-21** (§18 R3). gamma and production therefore run `sandbox.gvisor.enabled: false` unless DigitalOcean ships native sandbox-runtime support; an open feature request has no official response. This is a platform constraint, not a schedule slip — see §19 Q6. |
...
**Alpha half done, 2026-09-20 — negative:** no `runsc`, no `RuntimeClass`, no containerd handler, and no `gvisor` core addon on MicroK8s v1.35.6 (§18, R3). The DigitalOcean half is still outstanding, and the alpha posture decision — installer DaemonSet versus `sandbox.gvisor.enabled: false` — is now a live call rather than a theoretical one.
...
| Q6 | **Does "gVisor by default" survive the platform findings?** (Raised 2026-09-21 by the §18 R3 verification.) D9/D10 make a gVisor `RuntimeClass` the default posture, but the alpha path documented in §12.2.1 does not exist, and DOKS — gamma and production — cannot host `runsc` at all, installer DaemonSet included. So the default posture is currently deliverable in **no** environment we run. Is the answer (a) build the installer and run gVisor on alpha only, accepting gamma/prod without it, (b) find a different sandbox for DOKS, or (c) drop gVisor from the default posture and re-rate the D9 threat model on the remaining layers? Needs a human decision — it touches an approved decision, not just a deployment detail. | `sandbox.gvisor.enabled: false` everywhere; every other sandbox layer (rootless, dropped caps, read-only rootfs, seccomp, default-deny NetworkPolicy, the WASM sandbox itself) stays on, and startup still fails closed where gVisor is explicitly requested. Note this also blocks R8's benchmark, which needs one real gVisor-enabled run. |
