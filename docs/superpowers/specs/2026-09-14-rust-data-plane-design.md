...
| `docs/plans/2026-08-31-v3-sccebm-program-plan.md` open item #9, lines 407-408 — *"native = first-party only; community-native deferred until real sandbox"* | **Closed by this spec.** Community-authored native bundles are admitted on the WASI 0.2 component sandbox plus the capability-scoped host interface (D33). gVisor is **not** part of that boundary — it is opt-in and off by default (D32). |
...
| D33 | **Marketplace and community-authored bundles are admitted on the WASM sandbox alone.** Following D32, the runtime boundary for third-party bundle code is the WASI 0.2 component sandbox plus the capability-scoped host interface — no user-space kernel beneath it. The containment that remains is real and unchanged: bundles hold no credentials, reach only declared `egress` hosts, run inside a credential-less executor Deployment under a default-deny NetworkPolicy, and are built by a compiler Job with **zero** network (nothing in or out) and **zero** mounted secrets. **Cost, stated plainly:** a wasmtime escape at runtime, or a container escape from the `build` container at compile time, now reaches the host kernel directly rather than a sentry (§11). | The alternative was re-deferring community-native to first-party-only. §18 R3 establishes that no environment Waddles runs can host gVisor, so "wait for a second wall" has no delivery date and would block the App marketplace indefinitely. | Human, 2026-09-22 |
| D34 | **Standing posture for untrusted bundle code is the defence-in-depth stack, judged sufficient for now — with a named escalation path.** The stack: WASM component sandbox + rootless/caps-dropped/read-only-rootfs/seccomp pods + default-deny `CiliumNetworkPolicy` + **enforced** Tetragon `TracingPolicy` (§11 layer 10). gVisor remains the opt-in per-cluster option (D32), not part of the default. The compiler additionally scans inert extracted source and requires all scans to pass **before** any step executes bundle code (§9.3 scan-before-execution ordering). **Escalation path, deferred:** Kata Containers (VM isolation) — unlike gVisor it has a real install path on our clusters (the `kata` MicroK8s community addon exists; gVisor's never did, §18 R3) — to be evaluated only if runtime enforcement proves insufficient. | gVisor is undeliverable in any environment Waddles runs (§18 R3), and re-deferring community/marketplace bundles until a second wall exists was already rejected (D33); the posture leans on runtime enforcement now, with a viable VM-isolation escalation named rather than left open-ended. | Human, 2026-09-22 |
| D35 | **Accepted bundle input languages: Python, Rust, JavaScript/NodeJS (Tier 1, spike-proven, unchanged from D11) plus C# (Tier 2, spike-pending) — all compiled to WASI 0.2 components.** Python, Rust and JS/TS toolchains were exercised end to end by the compiler-sandbox and `penguin-dal`-in-WASM spikes (§18 R1/R2) and stay Tier 1. C# is accepted as a target — `componentize-dotnet` / NativeAOT-LLVM — but no spike has exercised that toolchain; it is Tier 2 (prebuilt-only, permanent "not security-scanned" badge per D12) until a feasibility spike (§18 R13) proves the component build path. `waddle-sdk-dotnet` is reserved as a package name only, marked pending-spike (§6.5). | Widens the authoring population to C# without claiming a toolchain works before it has been tested — the same gating discipline R1/R2 already applied to the Python facade. | Human, 2026-09-22 |
...
| C# (`componentize-dotnet` / NativeAOT-LLVM) — **spike-pending, D35** | TBD pending spike | TBD pending spike |
...
**C# SDK placeholder (D35).** `waddle-sdk-dotnet` is reserved as the package name for a future Tier 1 C# SDK but ships no compiler recipe, build-flag row or example yet — its allowlist row above reads `TBD pending spike` because no feasibility spike (§18 R13) has run. Do not build against it, and do not treat C# as a working toolchain, until R13 closes.
...
**Scan-before-execution ordering (D34).** The static and malware scans above — `semgrep`, the dependency audit, `gitleaks`, and Skauswatch — run against the inert extracted source tarball and **must all pass before** the pipeline runs any step that executes bundle code (the Python `pkgutil.walk_packages` pre-import and `componentize-py`, both of which run guest top-level code, §18 R2). Untrusted code is scanned before it is ever executed, even inside the build sandbox; a scan failure blocks here and the build container never runs the bundle.
...
| **Hostile community bundle author** (D33) | The author population is untrusted by design. At runtime the bundle is a WASI 0.2 component holding no credentials, able to reach only the `egress` hosts its manifest declares and approved at install (§9.7), executing DB statements only through the table-allowlisted, RLS-scoped `db` host call under its own Postgres role. Install-time review and the permission-consent screen gate what it may ask for at all. | The WASM boundary itself. With gVisor off by default (D32) there is no second wall beneath it, so a wasmtime escape is contained by seccomp, dropped capabilities, the credential absence and the network policy — not by a user-space kernel. |
| **Hostile bundle source at _build_ time** (D33) | The sharpest edge of admitting third-party authors: `componentize-py` executes bundle top-level code during compilation (§18 R2), so the `build` container runs genuinely arbitrary code, **not** WASM-sandboxed code. Containment is that it has **no network at all** (the NetworkPolicy allows nothing in or out), **no mounted secrets**, a read-only rootfs, dropped capabilities and `RuntimeDefault` seccomp, and is an ephemeral Job pod; the digest is computed in the separate trusted `publisher` container (D27). | A host-kernel container escape from an ephemeral, network-isolated, credential-less pod. Previously this was the one place gVisor was load-bearing; with D32 it is opt-in, so where it is off this escape reaches the host kernel. Highest-value target of the D33 decision — review here before relaxing anything else. |
...
| 3 | **gVisor `RuntimeClass` (`runsc`) — optional, off by default everywhere (D32).** Where enabled, syscalls are serviced by a user-space kernel, verified at startup (§12.2); §18 R3 found no environment Waddles runs can sustain it as a default (no addon on MicroK8s, not survivable on DOKS) | A native-code escape reaching the host kernel's syscall surface — **when enabled.** Absent by default, so layers 2 and 10 (pod security + enforced Tetragon) are the operative boundary against this specific threat in every environment we run today |
...
| 10 | **Tetragon `TracingPolicy` (enforced, not observe-only)** — a runtime binary/syscall allowlist on the executor and compiler pods per `security.md`; an unexpected `exec` or syscall is killed and counted | A native-code escape past the WASM boundary that tries to run tooling or syscalls outside the bundle's profile — the runtime-enforcement layer that most directly compensates for gVisor being off by default (D32/D34) |
...
| R13 | **C# → WASM component feasibility (D35).** The `componentize-dotnet` / NativeAOT-LLVM toolchain for compiling C# to a WASI 0.2 component implementing the `waddle:bundle/stage` world has never been exercised — unlike Python and Rust, nothing about it is confirmed working (§18 R1/R2 close that gap for the Tier 1 languages; this row is the same gate for C#). | Medium: a real toolchain gap could block C# entirely or force a different compilation path. | A feasibility spike, mirroring R1/R2's gating discipline, must confirm build, the per-language import-allowlist shape (§6.5) and WIT-world conformance before C# moves out of Tier 2 / prebuilt-only. | **Not started** — gates enabling C# as more than a manually-built Tier 2 upload. |
